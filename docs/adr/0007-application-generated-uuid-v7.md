...
- **다른 모듈의 ID는 `UUID`로 들고 있는다.** 다른 모듈의 값 클래스는 내부 타입이라 참조할 수 없다 (ADR-0003). 값 클래스는 바이트코드에서 `UUID`로 지워져 Modulith가 이 위반을 잡지 못하므로, `SourceImportRulesTests`가 import 문으로 검사한다.
