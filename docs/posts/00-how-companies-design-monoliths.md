...
| #4 | 운영 가능한 서비스의 조건 (보안, 관측성, CI/CD) |
| #5~6 | 성능 기준선 측정과 튜닝 |
| #7~ | 모듈 추출, DB 분리, Saga와 아웃박스, 장애 격리, 전후 비교 |
...
**정말 그렇게 되는지는 #7 이후에 직접 확인할 예정이다.** 어댑터만 바꾸면 끝난다는 건 설계상의 희망일 뿐이고, 실제로는 트랜잭션 경계와 네트워크 지연이라는 문제가 남을 거라고 예상한다.
